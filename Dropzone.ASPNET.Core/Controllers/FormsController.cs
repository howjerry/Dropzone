...
namespace Dropzone.ASPNET.Core.Controllers
{
    public class FormsController : Controller
    {
        ...
        public FormsController(IWebHostEnvironment hostEnvironment)
        {
        ...
        public IActionResult Upload(FormsUploadRequest request)
        {
            try
            {
                ...
                foreach (IFormFile file in files)
                {
                    if (file != null && file.Length > 0)
                    {
                        ...
                        string fileNewName = request.UUIDs[index] + fileExtension;
